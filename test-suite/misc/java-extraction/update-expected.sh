...
run_one() {
  ...
  if [ -x "../_build/install/default/bin/rocq" ] &&
    ...
    printf 'missing built Rocq; build this checkout before updating expected files\n' >&2
    exit 1
  fi
  ...
  if [ "$found" = false ]; then
    printf 'no Java files generated for case: %s\n' "$case_name" >&2
    exit 1
  fi
  ...
  for generated in "$generated_dir"/*.java; do
    cp "$generated" "$expected_dir/$(basename "$generated").expected"
  done
...
case $1 in
  all)
    for case_path in "$self_dir"/*.v; do
      run_one "$(basename "$case_path" .v)"
    done
    ;;
  *)
    run_one "$1"
